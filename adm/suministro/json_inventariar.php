<?php
    ...
    $id_categoria = $suministro ->get_categoria_resume_name_2($categoria);
    ...
    foreach ($detail as $valor){
        for($i=1; $i <= $valor["restante"]; $i++){
            ...
            $data[] = array("status" => status($numer_cod_articulo),
                            ...
                            "accion" => accion($cod_articulo,$valor["id_factura_detalle"],$valor["id_factura"],$numer_cod_articulo,$id_categoria));
        }
    }
    ...
    function no_inventario($numer_cod_articulo){
        $Y = date("Y");
        return "<input type='text' class='form-control form-control-sm font-weight-semibold text-blue-800 text-uppercase' id='inv_$numer_cod_articulo' value='SP-$numer_cod_articulo-$Y'>";
    }
