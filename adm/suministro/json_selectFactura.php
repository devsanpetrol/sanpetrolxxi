<?php
    ...
    $categorias = $suministro->get_doctoFactura("WHERE date_insert BETWEEN DATE_ADD(date(now()), INTERVAL(-WEEKDAY(date(now()))) DAY) AND DATE_ADD(date(now()), INTERVAL (6 -WEEKDAY(date(now())) ) DAY)");
    $data = array();
